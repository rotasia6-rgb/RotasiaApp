...
export default function BestAttirePage() {
    ...
    const [newEntry, setNewEntry] = useState({ name: '', caption: '', gender: 'male' as 'male' | 'female', file: null as File | null });
    ...
    const fetchEntries = async () => {
        try {
            // Fetch User's own submissions or all approved ones?
            // Requirement: "show the data on the management page for approval once its approved it should be visibloe on the voting poll"
            // For this public page, maybe we want to show approved ones similarly to voting page? 
            // Or is this page just for submission? 
            // Assuming this page shows the gallery of approved nominations for "Best Attire" specifically (if we treat it as a category).
            // But for now, let's fetch all approved nominations to display the "Gallery".
    ...
    const handleLike = async (item: Nomination) => {
        ...
        setEntries(prev => prev.map(e => e.id === item.id ? { ...e, votes: newVotes } : e));
        ...
        // DB Update
        try {
            const { error } = await supabase.rpc('increment_vote', { row_id: item.id });
            if (error) {
                // Fallback
                console.warn("RPC failed, trying direct update", error);
                await supabase.from('nominations').update({ votes: newVotes }).eq('id', item.id);
            }
        } catch (err) {
            console.error(err);
        }
    ...
    const handleShare = async (item: Nomination) => {
        const url = `${window.location.origin}/best-attire?id=${item.id}`;
        const shareData = {
            title: `Check out ${item.contestant_name}'s outfit!`,
            text: `Vote for ${item.contestant_name} on Rotasia Best Attire!`,
            url: url
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error: dbError } = await supabase
                .from('nominations')
                .insert({
                    contestant_name: newEntry.name,
                    caption: newEntry.caption,
                    contestant_photo: publicUrl,
                    status: 'pending',
                    gender: newEntry.gender,
                    category: 'best_attire' // Or whatever category logic is desired
                });
            ...
            setNewEntry({ name: '', caption: '', gender: 'male', file: null });
            ...
            <main className="max-w-6xl mx-auto pt-4 md:pt-10">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/10">
                        ...
                        <p className="text-gray-500 mt-2">Entries will appear here once approved.</p>
                    </div>
                ) : (
                        ...
                        {entries.map((item) => (
                            ...
                            >
                                ...
                                <div className="p-4 relative -mt-20">
                                    <h3 className="text-xl font-bold text-white mb-1 truncate shadow-black drop-shadow-md">{item.contestant_name}</h3>
                                    ...
                                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span className="font-bold text-lg tabular-nums">{item.votes || 0}</span>
                                        </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-4">
                                        <label className={cn(
                                            "flex-1 border border-white/10 rounded-lg p-3 flex items-center justify-center gap-2 cursor-pointer transition-all",
                                            newEntry.gender === 'male' ? "bg-blue-500/20 border-blue-500 text-blue-400" : "bg-white/5 hover:bg-white/10 text-gray-400"
                                        )}>
                                            <input
                                                type="radio"
                                                name="gender"
                                                value="male"
                                                checked={newEntry.gender === 'male'}
                                                onChange={() => setNewEntry({ ...newEntry, gender: 'male' })}
                                                className="hidden"
                                            />
                                            <span className="font-bold">Mr. Rotasia</span>
                                        </label>
                                        <label className={cn(
                                            "flex-1 border border-white/10 rounded-lg p-3 flex items-center justify-center gap-2 cursor-pointer transition-all",
                                            newEntry.gender === 'female' ? "bg-pink-500/20 border-pink-500 text-pink-400" : "bg-white/5 hover:bg-white/10 text-gray-400"
                                        )}>
                                            <input
                                                type="radio"
                                                name="gender"
                                                value="female"
                                                checked={newEntry.gender === 'female'}
                                                onChange={() => setNewEntry({ ...newEntry, gender: 'female' })}
                                                className="hidden"
                                            />
                                            <span className="font-bold">Ms. Rotasia</span>
                                        </label>
                                    </div>
                                </div>
